...
u_data['timestamp'] = pd.to_datetime(u_data['timestamp'])
u_data['timestamp'] = u_data['timestamp'].dt.date

u_item = pd.read_csv('ml-100k/u.item', sep = '|', encoding = 'latin-1', header = None, 
...
# item_id --> constitute the rows 
# user_id --> constitute the columns 
# rating --> value inside the matrix
...
# Creating a similarity matrix based on genre
...
def get_similar_items(item_id, k=10, min_ratings = 5): 
    ...
    # calculates the number of non-zero ratings for each item by creating a Boolean mask where 'True' values indicate positive ratings (>0)
    # It then sums up the 'True' values along each row (axis = 1) of the matrix. row ==> item_id 
...
try: 
    ...
    st.write(f"Recommendation are not available for '{selected_movie_title}'.")
